...
export function lipstickTheoryBonus(
  ...
  // Pastel / baby pink names — actively avoid even when hex is borderline.
  if (
    /(baby\s*pink|ballet|candy|petal|soft\s*pink|light\s*pink|roze|pink\b)/u.test(name) &&
    shade.lab.L > 45
  ) {
    bonus -= 14
  }
